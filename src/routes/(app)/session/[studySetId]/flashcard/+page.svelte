...
<div>
	<div class="mx-auto flex w-full max-w-2xl flex-col gap-3 px-6 py-3">
		...
		<div class="flex flex-col gap-3 rounded-4xl bg-card px-4 py-4 text-card-foreground shadow-xs">
			<div class="flex items-end justify-between gap-3 px-1">
				<div>
					<h2 class="font-semibold">Seberapa Paham Kamu?</h2>
					<p class="text-xs text-muted-foreground">Pilih untuk menjadwalkan pengulangan.</p>
				</div>
				<div>
					<Popover>
						<PopoverTrigger class={buttonVariants({ size: 'icon-sm', variant: 'outline' })}>
