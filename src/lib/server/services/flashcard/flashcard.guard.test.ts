...
describe.concurrent(FlashcardGuard, () => {
  describe("assertStudySetOwnerOrForbidden", () => {
    it("returns the study set when the caller owns it", async ({ expect }) => {
      ...
      expect(studySet.assertStudySetOwnerOrForbidden).toHaveBeenCalledWith(
        "set-1",
        "owner-1"
      );
    });
  ...
  describe("assertStudySetVisibleOrNotFound", () => {
    it("returns the study set to the owner even when PRIVATE", async ({
      expect,
    }) => {
      ...
      expect(result).toBe(set);
      expect(studySet.assertStudySetVisibleByIdOrNotFound).toHaveBeenCalledWith(
        "set-1",
        "owner-1"
      );
    });
    ...
    it("throws NOT_FOUND for a private set viewed by a non-owner", async ({
      expect,
    }) => {
      const { studySet, guard } = setupGuard();
  ...
  describe("assertChapterOwnerInStudySetOrForbidden", () => {
    it("returns when the chapter belongs to the study set and is owned", async ({
      expect,
    }) => {
    ...
    it("throws FORBIDDEN when the chapter is owned by a different user", async ({
      expect,
    }) => {
      const { repo, guard } = setupGuard();
      repo.findChapter.mockResolvedValue({
  ...
  describe("assertFlashcardOwnerOrForbidden", () => {
    it("returns the card when the caller is the owner", async ({ expect }) => {
      ...
      const result = await guard.assertFlashcardOwnerOrForbidden(
        "card-1",
        "owner-1"
      );
    ...
    it("throws FORBIDDEN when caller is not the owner", async ({ expect }) => {
      ...
      await expect(
        guard.assertFlashcardOwnerOrForbidden("card-1", "other")
  ...
  describe("assertFlashcardVisibleOrNotFound", () => {
    it("returns the card to the owner even when the study set is PRIVATE", async ({
      expect,
    }) => {
      ...
      const result = await guard.assertFlashcardVisibleOrNotFound(
        "card-1",
        "owner-1"
      );
      expect(result).toBe(card);
    ...
    it("returns the card to a non-owner when the study set is PUBLIC", async ({
      expect,
    }) => {
      const card = createFlashcardFixture({
        id: "card-1",
    ...
    it("throws NOT_FOUND for a non-owner viewing a card in a PRIVATE set", async ({
      expect,
    }) => {
      const card = createFlashcardFixture({
        id: "card-1",
    ...
    it("throws NOT_FOUND when the parent study set is missing", async ({
      expect,
    }) => {
      const card = createFlashcardFixture({
        id: "card-1",
  ...
  describe("assertFlashcardsAllOwnedOrThrow", () => {
    ...
    it("throws PARTIAL_FORBIDDEN listing all unowned ids", async ({
      expect,
    }) => {
      const { repo, guard } = setupGuard();
      repo.findFlashcardsByIds.mockResolvedValue([
