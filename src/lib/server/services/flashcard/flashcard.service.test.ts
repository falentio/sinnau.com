...
const setupService = () => {
  ...
  guard.assertChapterOwnerInStudySetOrForbidden.mockResolvedValue();
  ...
  guard.assertFlashcardsAllOwnedOrThrow.mockResolvedValue();
...
describe.concurrent(FlashcardService, () => {
  describe("createFlashcards", () => {
    ...
    it("creates one flashcard with generated id, default importance, null hint", async ({
      ...
      expect(
        guard.assertChapterOwnerInStudySetOrForbidden
      ...
      expect(rows[0]?.id).toMatch(/^fcd_[a-z0-9]{2}[A-Za-z0-9]{16}$/u);
    ...
    it("validates every unique chapterId exactly once and aborts on the first mismatch", async ({
      ...
      guard.assertChapterOwnerInStudySetOrForbidden.mockImplementation(
      ...
      const { calls } = guard.assertChapterOwnerInStudySetOrForbidden.mock;
    ...
    it("stores hint string when provided", async ({ expect }) => {
      ...
      await service.createFlashcards(
        {
          flashcards: [{ back: "B", front: "F", hint: "a small hint" }],
          studySetId: sampleStudySetId,
        },
        "owner-1"
      );
  ...
  describe("updateFlashcard", () => {
    it("propagates FORBIDDEN from assertFlashcardOwnerOrForbidden", async ({
      expect,
    }) => {
      const { repo, guard, service } = setupService();
    ...
    it("updates front+back and clears hint when set to null", async ({
      expect,
    }) => {
      const { repo, guard, service } = setupService();
      guard.assertFlashcardOwnerOrForbidden.mockResolvedValue(
      ...
      await service.updateFlashcard(
        { back: "B2", front: "F2", hint: null, id: "card-1" },
        "owner-1"
      );
      expect(repo.updateFlashcard).toHaveBeenCalledWith(
        "card-1",
        "owner-1",
        expect.objectContaining({ back: "B2", front: "F2", hint: null })
      );
    });
    ...
    it("omits hint and importance from patch when not provided", async ({
      expect,
    }) => {
      const { repo, service } = setupService();
      // oxlint-disable-next-line require-await
  ...
  describe("deleteFlashcards", () => {
    it("propagates PARTIAL_FORBIDDEN from assertFlashcardsAllOwnedOrThrow", async ({
      expect,
    }) => {
      const { repo, guard, service } = setupService();
      guard.assertFlashcardsAllOwnedOrThrow.mockImplementation(
        throwPartialForbidden
      );
      const err = await captureError(
        service.deleteFlashcards({ ids: ["a", "b"] }, "owner-1")
      );
      expect(err).toBeInstanceOf(ORPCError);
    ...
    it("deletes all ids when the ownership check passes", async ({
      expect,
    }) => {
      const { repo, service } = setupService();
      await service.deleteFlashcards({ ids: ["a", "b", "c"] }, "owner-1");
      expect(repo.deleteFlashcards).toHaveBeenCalledWith(
        ["a", "b", "c"],
        "owner-1"
      );
    });
  ...
  describe("getFlashcards", () => {
    it("propagates NOT_FOUND from the study set visibility check", async ({
      expect,
    }) => {
      const { repo, guard, service } = setupService();
    ...
    it("returns the repository result for a visible study set", async ({
      expect,
    }) => {
      const { repo, service } = setupService();
      const cards = [
      ...
      expect(repo.findFlashcardsByStudySet).toHaveBeenCalledWith(
        sampleStudySetId
      );
  ...
  describe("getFlashcard", () => {
    it("returns the flashcard returned by the guard", async ({ expect }) => {
      ...
      expect(guard.assertFlashcardVisibleOrNotFound).toHaveBeenCalledWith(
        "card-1",
        "owner-1"
      );
    ...
    it("propagates NOT_FOUND from the visibility check", async ({ expect }) => {
      ...
      const err = await captureError(
        service.getFlashcard({ id: "card-1" }, "user-1")
      );
      expect(err).toMatchObject({ code: "NOT_FOUND" });
    });
  });
