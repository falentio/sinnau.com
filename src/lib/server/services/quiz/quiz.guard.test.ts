...
function setupGuard() {
	...
	return { repo, studySetGuard, chapterGuard, guard };
}
...
describe.concurrent('QuizGuard', () => {
	describe('assertStudySetOwnerOrForbidden', () => {
		it('returns the set when the caller is the owner', async ({ expect }) => {
			...
			expect(studySetGuard.assertStudySetOwnerOrForbidden).toHaveBeenCalledWith('set-1', 'owner-1');
	...
	describe('assertStudySetVisibleOrNotFound', () => {
		it('delegates to the study set guard', async ({ expect }) => {
			...
			const result = await guard.assertStudySetVisibleOrNotFound('set-1', 'user-1');
			expect(studySetGuard.assertStudySetVisibleByIdOrNotFound).toHaveBeenCalledWith(
				'set-1',
				'user-1'
			);
			expect(result).toBe(set);
	...
	describe('assertChapterOwnerOrForbidden', () => {
		it('delegates to the chapter guard', async ({ expect }) => {
			...
			const result = await guard.assertChapterOwnerOrForbidden('ch-1', 'owner-1');
			expect(chapterGuard.assertOwnerOrForbidden).toHaveBeenCalledWith('ch-1', 'owner-1');
	...
	describe('assertChapterInStudySetOrForbidden', () => {
		it('returns the chapter when it belongs to the expected study set and is owned', async ({
			...
			const result = await guard.assertChapterInStudySetOrForbidden('ch-1', 'owner-1', 'set-1');
			expect(result).toBe(ch);
		});
		...
		it('throws NOT_FOUND when the chapter does not exist', async ({ expect }) => {
			...
			let err: unknown = null;
			try {
				await guard.assertChapterInStudySetOrForbidden('missing', 'owner-1', 'set-1');
			} catch (e) {
				err = e;
			}
			expect(err).toBeInstanceOf(ORPCError);
		...
		it('throws VALIDATION_FAILED when the chapter belongs to another study set', async ({
			...
			let err: unknown = null;
			try {
				await guard.assertChapterInStudySetOrForbidden('ch-1', 'owner-1', 'set-1');
			} catch (e) {
				err = e;
			}
			expect(err).toBeInstanceOf(ORPCError);
		...
		it('throws FORBIDDEN when the chapter is owned by another user', async ({ expect }) => {
			const { repo, guard } = setupGuard();
			repo.findChapterById.mockResolvedValue({
				id: 'ch-1',
	...
	describe('assertQuizOwnerOrForbidden', () => {
		...
		it('throws FORBIDDEN when caller is not the owner', async ({ expect }) => {
			const { repo, guard } = setupGuard();
			repo.findQuizById.mockResolvedValue(createQuizFixture({ id: 'q-1', ownerId: 'owner-1' }));
			let err: unknown = null;
			try {
	...
	describe('assertQuizOwnerBatchOrPartialForbidden', () => {
		it('returns all quizzes when all are owned', async ({ expect }) => {
			...
			const result = await guard.assertQuizOwnerBatchOrPartialForbidden(['q-1', 'q-2'], 'owner-1');
			expect(result).toEqual([a, b]);
		});

		it('throws PARTIAL_FORBIDDEN with blocked ids when one is not owned', async ({ expect }) => {
			const { repo, guard } = setupGuard();
			...
			let err: unknown = null;
			try {
				await guard.assertQuizOwnerBatchOrPartialForbidden(['q-1', 'q-2'], 'owner-1');
			} catch (e) {
				err = e;
			}
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'PARTIAL_FORBIDDEN', data: { ids: ['q-2'] } });
		});
	...
	describe('assertQuizOptionOwnerBatchOrPartialForbidden', () => {
		it('returns the owned options when all ids belong to the owner', async ({ expect }) => {
			const { repo, guard } = setupGuard();
			const owned = [{ id: 'o-1', quizId: 'q-1' } as never, { id: 'o-2', quizId: 'q-1' } as never];
			...
			expect(repo.findOptionsByIdsForOwner).toHaveBeenCalledWith(['o-1', 'o-2'], 'owner-1');
		});
		...
		it('throws PARTIAL_FORBIDDEN with the missing ids', async ({ expect }) => {
			...
			let err: unknown = null;
			try {
				await guard.assertQuizOptionOwnerBatchOrPartialForbidden(['o-1', 'o-2'], 'owner-1');
			} catch (e) {
				err = e;
			}
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'PARTIAL_FORBIDDEN', data: { ids: ['o-2'] } });
		});
	});

	describe('assertQuizVisibleByIdOrNotFound', () => {
		it('returns the quiz to the owner even if study set is private', async ({ expect }) => {
			...
			const result = await guard.assertQuizVisibleByIdOrNotFound('q-1', 'owner-1');
			expect(result).toBe(quizRow);
		});

		it('returns the quiz to any user when the study set is public', async ({ expect }) => {
			const { repo, studySetGuard, guard } = setupGuard();
			const quizRow = createQuizFixture({
				id: 'q-1',
				ownerId: 'owner-1',
			...
			const result = await guard.assertQuizVisibleByIdOrNotFound('q-1', 'other');
			expect(result).toBe(quizRow);
		});
		...
		it('rethrows NOT_FOUND with the quiz message when the parent study set is not visible', async ({
			...
			repo.findQuizById.mockResolvedValue(
				createQuizFixture({ id: 'q-1', ownerId: 'owner-1', studySetId: 'set-1' })
			);
