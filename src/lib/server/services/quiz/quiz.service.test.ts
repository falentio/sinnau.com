...
function setupService() {
	...
	guard.assertQuizOptionOwnerBatchOrPartialForbidden.mockResolvedValue([ownedOption]);
	...
	return { repo, guard, service, ownedQuiz, ownedOption };
}
...
describe.concurrent('QuizService', () => {
	describe('createQuiz', () => {
		...
		it('validates chapter ownership when chapterId is provided', async ({ expect }) => {
		...
		it('skips chapter validation when chapterId is omitted', async ({ expect }) => {
			const { guard, service } = setupService();
			await service.createQuiz(
				{
					studySetId: STUDY_SET_ID,
		...
		it('rejects MCQ with more than one correct option during creation', async ({ expect }) => {
				...
				service.createQuiz(
					{
						...
						type: 'FILL_IN_THE_BLANK',
						questionText: 'Q?',
						options: [{ optionText: 'A', isCorrect: false }]
					},
					'owner-1'
				)
			);
	...
	describe('updateQuiz', () => {
		it('propagates FORBIDDEN from assertQuizOwnerOrForbidden', async ({ expect }) => {
			const { repo, guard, service } = setupService();
		...
		it('updates only questionText and returns the hydrated quiz', async ({ expect }) => {
			const { repo, service } = setupService();
			const result = await service.updateQuiz({ id: QUIZ_ID, questionText: 'Updated?' }, 'owner-1');
	...
	describe('deleteQuizzes', () => {
		it('propagates PARTIAL_FORBIDDEN from the guard', async ({ expect }) => {
			const { repo, guard, service } = setupService();
			guard.assertQuizOwnerBatchOrPartialForbidden.mockImplementation(throwPartialForbidden);
			const err = await captureError(service.deleteQuizzes({ ids: [QUIZ_ID, 'other'] }, 'owner-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'PARTIAL_FORBIDDEN' });
			expect(repo.deleteQuizzes).not.toHaveBeenCalled();
		});

		it('passes ids and owner to the repo when all ids are owned', async ({ expect }) => {
			const { repo, service } = setupService();
			await service.deleteQuizzes({ ids: [QUIZ_ID, 'q-2'] }, 'owner-1');
			expect(repo.deleteQuizzes).toHaveBeenCalledWith([QUIZ_ID, 'q-2'], 'owner-1');
		});

		it('throws NOT_FOUND when the repo reports not all rows were deleted', async ({ expect }) => {
			const { repo, service } = setupService();
			repo.deleteQuizzes.mockResolvedValue(false);
			const err = await captureError(service.deleteQuizzes({ ids: [QUIZ_ID] }, 'owner-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'NOT_FOUND' });
		});
	});
	...
	describe('createQuizOptions', () => {
		it('rejects MCQ with a second correct option', async ({ expect }) => {
			...
			repo.findOptionsByQuizIds.mockResolvedValue([
				createQuizOptionFixture({ quizId: QUIZ_ID, isCorrect: true })
			]);
			const err = await captureError(
				service.createQuizOptions(
					{
						options: [
							{ quizId: QUIZ_ID, optionText: 'A', isCorrect: true },
							{ quizId: QUIZ_ID, optionText: 'B', isCorrect: false }
						]
					},
					'owner-1'
				)
		...
		it('rejects MS with all options incorrect', async ({ expect }) => {
			...
			const err = await captureError(
				service.createQuizOptions(
					{
						options: [
							{ quizId: QUIZ_ID, optionText: 'A', isCorrect: false },
							{ quizId: QUIZ_ID, optionText: 'B', isCorrect: false }
						]
					},
					'owner-1'
				...
				service.createQuizOptions(
					{
						options: [
							{ quizId: QUIZ_ID, optionText: 'A', isCorrect: true },
							{ quizId: QUIZ_ID, optionText: 'B', isCorrect: true }
						]
					},
					'owner-1'
				)
	...
	describe('updateQuizOption', () => {
		...
		it('rejects MCQ attempting to mark a second option correct', async ({ expect }) => {
		...
		it('rejects MS removing the last correct option', async ({ expect }) => {
			...
			repo.findOptionByIdForOwner.mockResolvedValue(ownedOption);
			repo.findQuizById.mockResolvedValue(ownedQuiz);
			repo.findOptionsByQuizIds.mockResolvedValue([{ ...ownedOption, isCorrect: true }]);
			const err = await captureError(
		...
		it('clears explanation when null is sent', async ({ expect }) => {
			...
			await service.updateQuizOption({ id: OPTION_ID, explanation: null }, 'owner-1');
		...
		it('throws NOT_FOUND when the repo update returns null', async ({ expect }) => {
			const { repo, service, ownedOption, ownedQuiz } = setupService();
			repo.findOptionByIdForOwner.mockResolvedValue(ownedOption);
			repo.findQuizById.mockResolvedValue(ownedQuiz);
			repo.findOptionsByQuizIds.mockResolvedValue([ownedOption]);
	...
	describe('deleteQuizOptions', () => {
		it('propagates PARTIAL_FORBIDDEN from the guard', async ({ expect }) => {
			const { repo, guard, service } = setupService();
			guard.assertQuizOptionOwnerBatchOrPartialForbidden.mockImplementation(throwPartialForbidden);
			const err = await captureError(
		...
		it('rejects MCQ removing the last correct option', async ({ expect }) => {
			...
			const err = await captureError(service.deleteQuizOptions({ ids: [OPTION_ID] }, 'owner-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'CANNOT_DELETE_LAST_CORRECT' });
		});
		...
		it('passes ids and owner to the repo when all checks pass', async ({ expect }) => {
			const { repo, service, ownedOption, ownedQuiz } = setupService();
			ownedOption.isCorrect = false;
			ownedQuiz.type = 'MULTIPLE_CHOICE';
			repo.findOptionsByIdsForOwner.mockResolvedValue([ownedOption]);
			...
			expect(repo.deleteQuizOptions).toHaveBeenCalledWith([OPTION_ID], 'owner-1');
		});
	...
	describe('getQuizzes', () => {
		it('returns an empty array when no quizzes exist', async ({ expect }) => {
			const { service } = setupService();
			const result = await service.getQuizzes({ studySetId: STUDY_SET_ID }, 'owner-1');
			expect(result).toEqual([]);
		});

		it('returns quizzes with embedded options', async ({ expect }) => {
			const { repo, service, ownedQuiz, ownedOption } = setupService();
			repo.findQuizzesByStudySetId.mockResolvedValue([{ ...ownedQuiz, options: [ownedOption] }]);
			const result = await service.getQuizzes({ studySetId: STUDY_SET_ID }, 'owner-1');
	...
	describe('getQuiz', () => {
		it('propagates NOT_FOUND from the guard', async ({ expect }) => {
			...
			const err = await captureError(service.getQuiz({ id: QUIZ_ID }, 'owner-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'NOT_FOUND' });
		});
