...
function setupService() {
	...
	repo.insertChapter.mockImplementation(async (row) => createChapterFixture(row));
...
function throwForbidden(): never {
	throw new ORPCError('FORBIDDEN', { message: 'Cannot modify a chapter you do not own' });
}
...
describe.concurrent('ChapterService', () => {
	describe('createChapter', () => {
		it('propagates FORBIDDEN from guard.assertStudySetOwnerOrForbidden', async ({ expect }) => {
			...
			const err = await captureError(
				service.createChapter({ studySetId: sampleStudySetId, title: 'Biology 101' }, 'owner-1')
			);
		...
		it('creates a chapter with a generated slug scoped to the study set', async ({ expect }) => {
			const { repo, guard, service } = setupService();
		...
		it('uses a short random slug when the sanitized title is too short', async ({ expect }) => {
		...
		it('consults the repo via isSlugTakenInStudySet with the studySetId for each candidate', async ({
			...
			const calls = repo.isSlugTakenInStudySet.mock.calls;
		...
		it('translates SlugConflictError into CHAPTER_SLUG_CONFLICT', async ({ expect }) => {
			const { repo, service } = setupService();
	...
	describe('updateChapter', () => {
		it('propagates FORBIDDEN from guard.assertOwnerOrForbidden', async ({ expect }) => {
			...
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'FORBIDDEN' });
			expect(guard.assertOwnerOrForbidden).toHaveBeenCalledWith(sampleChapterId, 'owner-1');
		...
		it('clears description when null is sent in update', async ({ expect }) => {
			...
			const result = await service.updateChapter(
				{ id: sampleChapterId, description: null },
				'owner-1'
			);
			expect(repo.updateChapter).toHaveBeenCalledWith(
				sampleChapterId,
		...
		it('returns the existing row without calling update when patch is empty', async ({
			...
			const result = await service.updateChapter({ id: sampleChapterId }, 'owner-1');
	...
	describe('deleteChapter', () => {
		it('propagates FORBIDDEN from guard.assertOwnerOrForbidden', async ({ expect }) => {
			const { repo, guard, service } = setupService();
			guard.assertOwnerOrForbidden.mockImplementation(throwForbidden);
			const err = await captureError(service.deleteChapter({ id: sampleChapterId }, 'owner-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'FORBIDDEN' });
			expect(guard.assertOwnerOrForbidden).toHaveBeenCalledWith(sampleChapterId, 'owner-1');
		...
		it('throws CHAPTER_NOT_EMPTY when the chapter contains flashcards', async ({ expect }) => {
			const { repo, service } = setupService();
		...
		it('passes the id and owner to the repo when owner matches and the chapter is empty', async ({
			...
			expect(repo.countChildren).toHaveBeenCalledWith(sampleChapterId);
			expect(repo.deleteChapter).toHaveBeenCalledWith(sampleChapterId, 'owner-1');
		});
	});
	...
	describe('getChapter', () => {
		it('delegates to guard.assertVisibleByIdOrNotFound and returns the row', async ({ expect }) => {
			const { guard, service } = setupService();
			const expected = createChapterFixture({ id: sampleChapterId, ownerId: 'owner-1' });
			guard.assertVisibleByIdOrNotFound.mockResolvedValue(expected);
			const result = await service.getChapter({ id: sampleChapterId }, 'user-1');
			expect(guard.assertVisibleByIdOrNotFound).toHaveBeenCalledWith(sampleChapterId, 'user-1');
		...
		it('propagates NOT_FOUND from the visibility check', async ({ expect }) => {
			...
			const err = await captureError(service.getChapter({ id: sampleChapterId }, 'user-1'));
			expect(err).toBeInstanceOf(ORPCError);
			expect(err).toMatchObject({ code: 'NOT_FOUND' });
		});
	});
