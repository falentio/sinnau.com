...
const setupGuard = () => {
  ...
  return { chapterRepo, guard, studySetRepo };
};

describe.concurrent(ChapterGuard, () => {
  describe("assertOwnerOrForbidden", () => {
    it("returns the chapter when the caller is the owner", async ({
      expect,
    }) => {
    ...
    it("throws FORBIDDEN when the caller is not the owner", async ({
      ...
      const err = await captureError(
        guard.assertOwnerOrForbidden("ch-1", "someone-else")
      );
      expect(err).toBeInstanceOf(ORPCError);
      expect(err).toMatchObject({ code: "FORBIDDEN" });
    });

    it("throws FORBIDDEN when the chapter does not exist", async ({
      expect,
    }) => {
      const { chapterRepo, guard } = setupGuard();
      chapterRepo.findChapterById.mockResolvedValue(null);
  ...
  describe("assertVisibleByIdOrNotFound", () => {
    ...
    it("returns the chapter to a non-owner when the study set is PUBLIC", async ({
      expect,
    }) => {
      const { chapterRepo, studySetRepo, guard } = setupGuard();
      const ch = createChapterFixture({
    ...
    it("throws NOT_FOUND when a non-owner requests a chapter in a PRIVATE study set", async ({
      ...
      const err = await captureError(
        guard.assertVisibleByIdOrNotFound("ch-1", "other-user")
      );
      expect(err).toBeInstanceOf(ORPCError);
    ...
    it("throws NOT_FOUND when the chapter is missing", async ({ expect }) => {
      ...
      chapterRepo.findChapterById.mockResolvedValue(null);
      const err = await captureError(
        guard.assertVisibleByIdOrNotFound("missing", "user-1")
      );
      expect(err).toBeInstanceOf(ORPCError);
      expect(err).toMatchObject({ code: "NOT_FOUND" });
    });
    ...
    it("throws NOT_FOUND when the parent study set is missing", async ({
      ...
      studySetRepo.findStudySetById.mockResolvedValue(null);
      const err = await captureError(
        guard.assertVisibleByIdOrNotFound("ch-1", "user-1")
      );
      expect(err).toBeInstanceOf(ORPCError);
      expect(err).toMatchObject({ code: "NOT_FOUND" });
    });
  ...
  describe("assertStudySetOwnerOrForbidden", () => {
    ...
    it("throws FORBIDDEN when the study set does not exist", async ({
      ...
      const err = await captureError(
        guard.assertStudySetOwnerOrForbidden("missing", "owner-1")
      );
      expect(err).toBeInstanceOf(ORPCError);
      expect(err).toMatchObject({ code: "FORBIDDEN" });
    });
