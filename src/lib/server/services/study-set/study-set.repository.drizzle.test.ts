...
import {
  studySet,
  studySetVisit,
} from "$lib/server/infras/db/schema/study-set";
...
describe.concurrent("StudySetDrizzleRepository", () => {
  ...
  describe("updateStudySet", () => {
    ...
    it("returns null when the id does not exist", async ({ expect }) => {
      await using env = new StudySetTestEnv();
      const result = await env.repo.updateStudySet("missing", env.ownerId, {
        title: "X",
      });
    ...
    it("returns null when ownerId does not match", async ({ expect }) => {
      ...
      const result = await env.repo.updateStudySet("set-1", env.otherId, {
        title: "Hacked",
      });
      expect(result).toBeNull();
  ...
  describe("findStudySetBySlug", () => {
    ...
    it("matches case-insensitively", async ({ expect }) => {
      ...
      const upper = await env.repo.findStudySetBySlug("MY-SET-ABC123");
      expect(upper?.id).toBe("set-1");
      const mixed = await env.repo.findStudySetBySlug("My-Set-Abc123");
      expect(mixed?.id).toBe("set-1");
    });
  ...
  describe("findOwnedStudySets", () => {
    ...
    it("paginates with a fixed limit of 10", async ({ expect }) => {
      ...
      const page1 = await env.repo.findOwnedStudySets(
        env.ownerId,
        "createdAt",
        "desc",
        1
      ...
      const page2 = await env.repo.findOwnedStudySets(
        env.ownerId,
        "createdAt",
        "desc",
        2
    ...
    it("orders by createdAt desc by default", async ({ expect }) => {
      await using env = new StudySetTestEnv();
      const first = await env.seedStudySet({
        id: "first",
        ownerId: env.ownerId,
      });
      await sleep(5);
      const second = await env.seedStudySet({
        id: "second",
        ownerId: env.ownerId,
      });
  ...
  describe("isSlugTaken", () => {
    ...
    it("returns true when the candidate matches case-insensitively", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      await env.seedStudySet({
  ...
  describe("upsertVisit", () => {
    it("inserts a new visit when none exists", async ({ expect }) => {
      ...
      const visit = await env.repo.upsertVisit(
        env.ownerId,
        created.id,
        visitedAt
      );
  ...
  describe("deleteOldVisits", () => {
    it("deletes only visits older than the cutoff and returns the count", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      ...
      expect(
        env.db
          .select()
          .from(studySetVisit)
          .where(eq(studySetVisit.id, "visit-old"))
          .all()
      ).toHaveLength(0);
    });

    it("returns 0 when no visits are older than the cutoff", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const created = await env.seedStudySet({
      ...
      await env.repo.upsertVisit(env.ownerId, created.id, Date.now());
      const deletedCount = await env.repo.deleteOldVisits(
        Date.now() - STUDY_SET_VISIT_TTL_MS
      );
  ...
  describe("findRecentVisits", () => {
    ...
    it("limits the result to the requested count", async ({ expect }) => {
      ...
      for (let i = 0; i < 5; i += 1) {
        // oxlint-disable-next-line no-await-in-loop -- test seeding requires sequential inserts for FK order
        const s = await env.seedStudySet({
          id: `s-${i}`,
          ownerId: env.ownerId,
        });
    ...
    it("includes private sets owned by the user themselves", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
    ...
    it("returns an empty array when the user has no visits", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const recent = await env.repo.findRecentVisits(env.otherId, 10);
...
describe.concurrent("StudySetDrizzleRepository (schema constraints)", () => {
  describe("slug uniqueness", () => {
    it("rejects inserting two rows with the same slug (case-insensitive)", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      await env.seedStudySet({
  ...
  describe("foreign keys", () => {
    it("rejects inserting a study set for a non-existent owner", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const insertOrphan = () =>
    ...
    it("rejects inserting a visit for a non-existent user", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const created = await env.seedStudySet({
  ...
  describe("visit unique constraint", () => {
    it("enforces (userId, studySetId) uniqueness at the DB level", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const created = await env.seedStudySet({
  ...
  describe("TTL cutoff boundary", () => {
    it("treats a visit at exactly the cutoff as not old", async ({
      expect,
    }) => {
      await using env = new StudySetTestEnv();
      const created = await env.seedStudySet({
