...
describe("getStudySetStubs", () => {
	...
	it("returns totalPages of 1 when count is less than limit", ({
		expect
	}) => {
	...
	it("assigns the given ownerId and shapes fields correctly", ({
		expect
	}) => {
	...
	it("makes every 5th item PRIVATE, rest PUBLIC (0-indexed)", ({
		expect
	}) => {
		...
		for (const [i, set] of result.studySets.entries()) {
			if (i % 3 === 0) {
				expect(set.description).toBe(
					`Deskripsi modul belajar nomor ${i + 1}`
				);
			} else {
