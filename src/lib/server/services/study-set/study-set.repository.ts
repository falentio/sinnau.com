...
export interface StudySetRepository {
	insertStudySet(row: Omit<StudySet, 'createdAt' | 'updatedAt'>): Promise<StudySet>;
	updateStudySet(id: string, ownerId: string, patch: StudySetUpdatePatch): Promise<StudySet | null>;
	...
	upsertVisit(userId: string, studySetId: string, visitedAt: number): Promise<StudySetVisit>;
