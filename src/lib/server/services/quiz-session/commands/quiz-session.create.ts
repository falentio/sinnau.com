...
const ERRORS = {
  FORBIDDEN: { message: "Cannot create a quiz session for this study set" },
  NOT_FOUND: { message: "Study set or chapter not found" },
  VALIDATION_FAILED: { message: "Chapter does not belong to the study set" },
} as const;
...
export const quizSessionCreate = authorizedProcedure
  ...
  .handler(
    async ({ input, context }) =>
      await quizSessionService.createQuizSession(input, context.user.id)
  );
