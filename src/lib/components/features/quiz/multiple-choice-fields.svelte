<script lang="ts">
  ...
  import { MCQ_OPTION_COUNT_HINT } from "$lib/schemas/quiz";
  ...
  const addOption = () => {
    if ($formData.options.length >= MCQ_OPTION_COUNT_HINT.max) {
      return;
    }
    $formData.options = [
      ...$formData.options,
  ...
  const removeOption = (index: number) => {
    if ($formData.options.length <= MCQ_OPTION_COUNT_HINT.min) {
      return;
    }
    const removedIsCorrect = ($formData.options as CreateQuizInput["options"])[
      index
    ]?.isCorrect;
    $formData.options = (
    ...
    if (
      removedIsCorrect &&
      !($formData.options as CreateQuizInput["options"]).some(
        (o) => o.isCorrect
      )
    ) {
      $formData.options = ($formData.options as CreateQuizInput["options"]).map(
        (opt, i) => (i === 0 ? { ...opt, isCorrect: true } : opt)
      );
    }
  };
  ...
  {#each $formData.options as _, i (i)}
    <div class="flex items-start gap-3 rounded-2xl border bg-background/50 p-3">
      ...
      <button
        ...
        disabled={disabled ||
          $formData.options.length <= MCQ_OPTION_COUNT_HINT.min}
...
{#if $formData.options.length < MCQ_OPTION_COUNT_HINT.max}
  <button
    ...
    {disabled}
  >
    + Tambah Opsi ({$formData.options.length}/{MCQ_OPTION_COUNT_HINT.max})
  </button>
{/if}
