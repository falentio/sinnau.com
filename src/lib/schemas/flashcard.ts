...
export const trimmedTextSchema = v.pipe(
  ...
  v.maxLength(
    FLASHCARD_TEXT_MAX_LENGTH,
    `Maksimal ${FLASHCARD_TEXT_MAX_LENGTH} karakter`
  )
);
...
export const importanceSchema = v.optional(
...
export const createFlashcardsInputSchema = v.object({
  flashcards: v.pipe(
    ...
    v.maxLength(
      FLASHCARD_BATCH_MAX,
      `Maksimal ${FLASHCARD_BATCH_MAX} flashcard per batch`
    )
  ),
...
export const deleteFlashcardsInputSchema = v.object({
  ids: v.pipe(
    ...
    v.maxLength(
      FLASHCARD_BATCH_MAX,
      `Maksimal ${FLASHCARD_BATCH_MAX} id per batch`
    )
  ),
});
...
export const flashcardDeleteOutputSchema = v.object({
  success: v.literal(true),
});
...
export type CreateFlashcardsInput = v.InferOutput<
  typeof createFlashcardsInputSchema
>;
export type UpdateFlashcardInput = v.InferOutput<
  typeof updateFlashcardInputSchema
>;
export type DeleteFlashcardsInput = v.InferOutput<
  typeof deleteFlashcardsInputSchema
>;
